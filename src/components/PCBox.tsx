...
// Theme configs for each generation
const generationThemes: Record<number, {
  name: string;
  frame: string;
  frameBorder: string;
  headerBg: string;
    ...
    <div className="flex flex-col items-center gap-4 w-full max-w-5xl">
      <div className="relative w-full">
        ...
        >
          ...
          >
            <div
              className="grid gap-1 p-2 rounded relative overflow-hidden"
              style={{
            ...
            >
              {paddedPokemon.map((pokemon, index) => (
                <div
                  ...
                  className="w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center relative z-10"
                >
                  {pokemon ? (
                    <Link href={`/pokemon/${pokemon.id}`}>
                      <div className="w-14 h-14 sm:w-[72px] sm:h-[72px] flex items-center justify-center hover:scale-125 transition-all cursor-pointer">
                        <Image
                          src={pokemon.sprite}
                          alt={pokemon.name}
                          width={72}
